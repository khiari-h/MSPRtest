...
abstract class viewGmp extends baseObjectGmp {
   ...
   public function addGmpNonce() {
    if (!$this->_nonceIsInited) {
        $this->_nonceIsInited = true;
        if (is_admin()) {
            $gmp_nonce = wp_create_nonce('gmp_nonce');
            $jsData = array(
            'gmp_nonce' => $gmp_nonce,
            );
            frameGmp::_()->addScript('nonceGmp', GMP_JS_PATH . 'nonce.js');
            frameGmp::_()->addJSVar('nonceGmp', 'GMP_NONCE', $jsData);
        } else {
            $gmp_nonce = wp_create_nonce('gmp_nonce_frontend');
            $jsData = array(
            'gmp_nonce_frontend' => $gmp_nonce,
            );
            frameGmp::_()->addScript('nonceGmp', GMP_JS_PATH . 'nonce.js');
            frameGmp::_()->addJSVar('nonceGmp', 'GMP_NONCE_FRONTEND', $jsData);
        }
    }
}
   static public function getAllowedHtml() {
      if (empty(self::$_allowedHtml)) {
         ...
         $allowedDiv = array(
           'style' => array(),
           'div' => array( 'hidden' => 1, 'data-show' => 1, 'data-section' => 1, 'data-slider-type' => 1, 'data-unit' => 1, 'data-unit' => 1, 'data-mapid' => 1, 'data-viewid' => 1, 'onclick' => 1, 'data-is-mobile' => 1, 'data-tab-link' => 1, 'data-tab-item' => 1, 'data-today' => 1, 'data-tabs-for' => 1, 'data-type' => 1, 'style' => 1, 'title' => 1, 'align' => 1, 'class' => 1, 'width' => 1, 'height' => 1, 'title' => 1, 'id' => 1, 'data-tooltip-content' => 1,),
