jQuery(document).ready(function(){
	...
	jQuery('#gmpMgrRemoveGroupBtn').click(function(){
		...
		if(confirm(confirmMsg)) {
			jQuery.sendFormGmp({
				btn: this
			,	data: {mod: 'marker_groups', action: 'removeGroup', _wpnonce: GMP_NONCE['gmp_nonce'], listIds: listIds}
	...
	jQuery('#gmpMgrClearBtn').click(function(){
		if(confirm(toeLangGmp('Clear whole marker categories list?'))) {
			jQuery.sendFormGmp({
				btn: this
			,	data: {mod: 'marker_groups', action: 'clear', '_wpnonce' : GMP_NONCE['gmp_nonce']}
			,	onSuccess: function(res) {
					if(!res.error) {
						location.reload();
					}
...
function gmpRemoveMarkerGroupFromTblClick(markerGroupId){
	...
	jQuery.sendFormGmp({
		msgElID: msgEl
	,	data: {action: 'remove', _wpnonce: GMP_NONCE['gmp_nonce'], mod: 'marker_groups', id: markerGroupId}
	,	onSuccess: function(res) {
			if(!res.error){
				location.reload();
			}
